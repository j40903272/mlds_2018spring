...
W_fc1 = weight_variable([14*14*1, 14])
b_fc1 = bias_variable([14])
h_pool1_flat = tf.reshape(h_pool1, [-1, 14*14*1])
h_fc1 = tf.nn.relu(tf.matmul(h_pool1_flat, W_fc1) + b_fc1)
h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)

W_fc2 = weight_variable([14, 13])
b_fc2 = bias_variable([13])
h_fc2 = tf.nn.relu(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)
h_fc2_drop = tf.nn.dropout(h_fc2, keep_prob)

W_fc3 = weight_variable([13, 10])
...
loss = np.array(loss)
acc = np.array(acc)
np.save('cnn_hidden2_loss_.npy', loss)
np.save('cnn_hidden2_acc_.npy', acc)
